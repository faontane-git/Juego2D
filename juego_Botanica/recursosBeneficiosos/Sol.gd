...
var picked = false
var sol_appear_timer := Timer.new()
var sol_disappear_timer := Timer.new()
var sol_object_lifetime = 5
var hud_height = 300
var is_visible = true

func _ready():
	randomize()
	add_child(sol_appear_timer)
	add_child(sol_disappear_timer)
	sol_appear_timer.connect("timeout", self, "_on_appear_timer_timeout")
	sol_disappear_timer.connect("timeout", self, "_on_disappear_timer_timeout")
	randomize_position()
...
func _on_appear_timer_timeout():
	...
	self.visible = true
	sol_appear_timer.stop()
	sol_disappear_timer.start(sol_object_lifetime)  # Desaparecer después de 5 segundos

func _on_disappear_timer_timeout():
	
	self.visible = false
	restablish()
	sol_disappear_timer.stop()
	sol_appear_timer.start(sol_object_lifetime)  # Volver a aparecer después de 5 segundos
func restablish():
	position.x= 0
	position.y=0
...
func _on_game_over():
	sol_appear_timer.stop()  # Detener los temporizadores al finalizar el juego
	sol_disappear_timer.stop()
	queue_free()
